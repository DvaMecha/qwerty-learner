      ...
      <DialogTrigger asChild>
        <div
          ...
          className={`group flex  h-36 w-80 cursor-pointer items-center justify-center overflow-hidden rounded-lg p-4 text-left shadow-lg focus:outline-none ${
            isSelected ? 'bg-cyan-400' : 'bg-zinc-50 hover:bg-white dark:bg-gray-800 dark:hover:bg-gray-700'
          }`}
          role="button"
          // onClick={onClick}
        >
          <div className="relative ml-1 mt-2 flex h-full w-full flex-col items-start justify-start">
            <h1
              className={`mb-1.5 text-xl font-normal  ${
                isSelected ? 'text-white' : 'text-gray-800 group-hover:text-cyan-400 dark:text-gray-200'
              }`}
            >
              {dictionary.name}
            </h1>
            ...
            <div className=" flex w-full items-center pt-2">
              {progress > 0 && (
                <Progress.Root
                  value={progress}
                  max={100}
                  className={`mr-4 h-2 w-full rounded-full border  bg-white ${isSelected ? 'border-cyan-600' : 'border-cyan-400'}`}
                >
                  <Progress.Indicator
                    className={`h-full rounded-full pl-0 ${isSelected ? 'bg-cyan-600' : 'bg-cyan-400'}`}
