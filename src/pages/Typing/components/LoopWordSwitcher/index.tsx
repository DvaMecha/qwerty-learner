  ...
  return (
    <>
      <Popover className="relative">
        <Popover.Button
          className={`p-[2px] ${
            loopTimes === 1 ? 'text-gray-500' : 'text-cyan-500'
          } rounded text-lg hover:bg-cyan-400 hover:text-white focus:outline-none `}
          type="button"
          onClick={(e) => {
            setIsOpen(!isOpen)
                  ...
                  >
                    {loopOptions.map((value, index) => (
                      <div className="flex w-full items-center" key={value}>
                        <RadioGroup.Item
                          className="h-[25px] w-[25px] cursor-pointer rounded-full bg-white shadow-[0_2px_10px]  shadow-gray-300 outline-none hover:bg-cyan-100"
                          value={value.toString()}
                          onClick={() => onChangeLoopTimes(value)}
                          id={`r${index}`}
                        >
                          <RadioGroup.Indicator className="relative flex h-full w-full items-center justify-center after:block after:h-[11px] after:w-[11px] after:rounded-[50%] after:bg-cyan-600 after:content-['']" />
                        </RadioGroup.Item>
                        <label
