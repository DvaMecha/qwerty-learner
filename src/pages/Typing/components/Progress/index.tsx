...
export default function Progress({ className }: { className?: string }) {
  ...
  const colorSwitcher: { [key: number]: string } = {
    0: 'bg-cyan-200 dark:bg-cyan-300',
    1: 'bg-cyan-300 dark:bg-cyan-400',
    2: 'bg-cyan-400 dark:bg-cyan-500',
  }
  ...
  return (
    <div className={`relative w-1/4 pt-1 ${className}`}>
      <div className="mb-4 flex h-2 overflow-hidden rounded-xl bg-cyan-100 text-xs transition-all duration-300 dark:bg-cyan-200">
        <div
          ...
          className={`flex flex-col justify-center whitespace-nowrap rounded-xl text-center text-white shadow-none transition-all duration-300 ${
            colorSwitcher[phase] ?? 'bg-cyan-200 dark:bg-cyan-300'
          }`}
        ></div>
      </div>
    </div>
