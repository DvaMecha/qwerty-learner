...
const RowPagination: FC<IRowPaginationProps> = ({ className, allRecords }) => {
  ...
  return (
    <div className={`-gap-1 flex select-none items-center ${className}`}>
      <button className="d cursor-pointer rounded-full  p-1  text-cyan-500 focus:outline-none dark:text-cyan-300" onClick={prevRowDetail}>
        <PrevIcon />
      </button>
      <span className="text-sm text-black dark:text-white">{`${currentIndex + 1} / ${allRecords.length}`}</span>
      <button className="cursor-pointer rounded-full p-1 text-cyan-500  focus:outline-none dark:text-cyan-300" onClick={nextRowDetail}>
        <NextIcon />
      </button>
    </div>
  )
