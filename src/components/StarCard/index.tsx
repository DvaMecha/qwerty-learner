          ...
          <div className="mt-6 flex w-full flex-col items-center gap-4">
            <div className="flex max-w-full items-center text-sm">
              ...
              <div className="flex flex-shrink-0 items-center">
                ...
                <IconStar className="h-4 w-4 text-cyan-600" />
            ...
            <span className="w-full text-center text-gray-600 dark:text-gray-300">
              收藏快捷键<span className="ml-2 text-cyan-600 dark:text-cyan-500">{IS_MAC_OS ? '⌘' : 'Ctrl'} + D</span>
            </span>
          </div>
        ) : (
          <div className="flex pb-0 pt-6">
            <button
              className="rounded-lg bg-cyan-600 px-6 py-2 text-lg text-white transition-colors duration-300 focus:outline-none"
              type="button"
        ...
        <div className="absolute right-3 top-3 flex h-5 items-center">
          {isCounting && (
            <span className="mx-1.5 dark:text-gray-100">
              <span className="text-cyan-600">{countdown}s </span>
              后自动关闭
            </span>
          )}
          <button type="button" onClick={onClickCloseStar} title="关闭提示" aria-label="关闭提示">
            <IconCircleX className="h-5 w-5 text-cyan-400" />
          </button>
        </div>
        <span className="pb-4 text-xl text-gray-600 dark:text-gray-50">
          坚持练习，提高语言能力。将 <span className="text-cyan-600">「Lucky Words」</span>保存到收藏夹，永不迷失！
        </span>
        {content}
      </div>
    </Transition>
